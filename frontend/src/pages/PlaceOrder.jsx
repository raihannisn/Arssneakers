    ...
    <div className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt14 min-h-[80vh] border-t'>
      ...
      <div className='mt-8'>
        ...
        <div className='flex gap-3 flex-col lg:flex-row'>
          <div onClick={()=>setMethod('stripe')} className='flex items-center gap-3 border border-gray-450 p-2 px-3 cursor-pointer'>
            <p className={`min-w-3.5 h-3.5 border border-gray-450 rounded-full ${method === 'stripe' ? 'bg-green-400' : ''}`}></p>
            <img className='h-5 mx-4' src={assets.stripe_logo} alt="" />
          </div>
          <div onClick={()=>setMethod('razorpay')} className='flex items-center gap-3 border border-gray-450 p-2 px-3 cursor-pointer'>
            <p className={`min-w-3.5 h-3.5 border border-gray-450 rounded-full ${method === 'razorpay' ? 'bg-green-400' : ''}`}></p>
            <img className='h-5 mx-4' src={assets.razorpay_logo} alt="" />
          </div>
          <div onClick={()=>setMethod('cod')} className='flex items-center gap-3 border border-gray-450 p-2 px-3 cursor-pointer'>
            <p className={`min-w-3.5 h-3.5 border border-gray-450 rounded-full ${method ===  'cod' ? 'bg-green-400' : ''}`}></p>
